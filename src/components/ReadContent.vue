...
export default {
    ...
    watch: {
        ...
        readContent: function () {//当阅读内容发生变化时
                console.log("scrollTop的值为", document.body.scrollTop)

            if (this.readContent.length === 1) {
                document.body.scrollTop = 0;//滚动条回到顶部
            }
        }
    },
    computed: {
    ...
    data() {
        return {
            isAdded: false,//是否在书架的指标
            isShowOpt: false,//展示几个栏
            isShowSet: false,//展示设置栏
            skinBgList: ['skin-default', 'skin-blue', 'skin-green', 'skin-pink', 'skin-dark', 'skin-light']
        }
    },
    created() {
		this.isAdded = this.curBook.isInShelf;//
        if (!this.skinBgList.includes(this.skinColor)) {//如果没有颜色
            this.SET_SKIN_COLOR('skin-default');//更新VUEX数据为默认
        }
    },
    methods: {
        ...mapMutations([
        ...
        showOpt() {
            if (this.isShowOpt || this.isShowSet) {
                ...
                this.isShowOpt = true;//全部关闭了，就把它打开
            }
        },
        showOptSet() {
            this.isShowSet = true;
		...
		addToShelf: function() {
			...
			this.ADD_TO_SHELF(book);//有本地缓存的，两个都会改变shelfbook的本地缓存
			this.isAdded = true;
		}
    }
}
